...
@property (nonatomic, strong) getHealthViewController *feature1VC;
...
- (void)setContainerViewController {
    getHealthViewController *vc1 = [getHealthViewController new];
    ...
    userCenterViewController *uc2 = [userCenterViewController new];
    
    [self.mContainerVCArr addObject:vc1];
    [self.mContainerVCArr addObject:uc2];
...
- (NSArray *)tabBarImages {
    return @[[UIImage imageNamed:@"down_button"], [NSNull null], [UIImage imageNamed:@"more@2x"]];
}

- (UIColor *)tabBarColor {
    return [UIColor colorWithRed:84.0f/255.0f
                           green:189.0f/255.0f
                            blue:140.0f/255.0f
                           alpha:1.0f];
}
...
- (void)setupNavigationStyle {
    NavigationTheme *theme = [[NavigationTheme alloc]
                              ...
                              //bar color
                              barColor:[UIColor colorWithRed:84.0f/255.0f
                                                       green:189.0f/255.0f
                                                        blue:140.0f/255.0f
                                                       alpha:1.0f]
                              //title font, size
    ...
    for (UIViewController *vc in self.mContainerVCArr) {
        if (i == button.tag) {
            ...
            switch (button.tag) {
                case 0:
                {
                    UIStoryboard *storyboard=[UIStoryboard storyboardWithName:@"getHealth" bundle:[NSBundle mainBundle]];
                    getHealthViewController *controllerD = [storyboard instantiateViewControllerWithIdentifier:@"health"];
...
- (void)updateContainerViewControllers {
    ...
    for (UIViewController *vc in self.mContainerVCArr) {
        ...
        if (![self.tabBarImages[i] isEqual:[NSNull null]]) {
            [button setImage:self.tabBarImages[i] forState:UIControlStateNormal];
        }
        if (i == 2) {
            button.imageEdgeInsets = UIEdgeInsetsMake(6, 2, 6, 10);
        }
        button.tag = i;
